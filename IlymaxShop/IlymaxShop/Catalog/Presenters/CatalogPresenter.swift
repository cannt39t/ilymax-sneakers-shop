...
class CatalogPresenter {
    ...
    public var pushListShoes: ([Shoes], String) -> Void = {_, _ in }
    ...
    func pushSearchShoeList(searchStr: String) {
        ...
        catalogService.getSearchShoes(searchStr: searchStr) { [weak self] shoes in
            DispatchQueue.main.async {
                self?.view?.hideLoader()
                self?.pushListShoes(shoes, "Search Result")
            }
        }
    }
}
